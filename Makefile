...
# project root (pyproject.toml, uv.lock, .venv) stays here so there is one venv
# for the repo -- hence `--app-dir backend` / `cd backend` below.
...
dev:
	$(UV) run uvicorn --app-dir backend server.main:app --reload --port $${SERVER_PORT:-8000} & \
...
prep:
	...
	cd backend && $(UV) run python -m prep.run_all --scene $(SCENE)
...
clean:
	rm -rf data/tmp .pytest_cache .ruff_cache frontend/dist
